...
if [ $? = 0 ] ; then
 ...
 ffmpeg -hide_banner -i "$INPUT" -filter_complex "volume=$gain, alimiter=limit=0.93:5:50" -metadata comment="" "$INPUT".tmp-limited.wav 
 ...
 ffmpeg -hide_banner -i "$INPUT".tmp-limited.wav -filter_complex "volume=$gain, silenceremove=0:0:0:-1:5:-92dB" -metadata comment="" -codec:a libmp3lame -b:a 320k "$OUTPUT" 
